...
同一套组件也可以作为右侧栏面板打开，这样在非日志页面上也能一直看到月历、进度条和语录。打开右侧栏，点击顶部的**方块**按钮，选择 **DB Banner**。它不是第二个横幅：没有壁纸，卡片单列堆叠；横幅是铺在照片之上的一层表面，所以配色写死，而侧栏面板处在页面之内，卡片跟随应用的明暗主题。点击某一天的跳转行为与横幅一致，底部的 **Banner settings / 横幅设置** 按钮会打开插件自己的设置面板。

注册走 `logseq.Experiments.registerSidebarRenderer`——插件能触及侧栏条目列表的唯一接口。@logseq/libs 0.2.11 还没有这个包装方法，因此 `main.ts` 通过 `invokeExperMethod` 调用同一个宿主方法。Logseq 在面板标题栏显示的是内部 renderer key 而不是注册的标题，这是宿主自己的渲染，插件无法改。
