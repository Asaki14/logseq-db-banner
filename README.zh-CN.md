...
点插件的工具栏图标（在工具栏插件弹层里固定后即可显示），图标下方会展开一个紧凑的设置弹层，包含下面所有设置项：它跟随 Logseq 当前主题，改动即时作用于横幅。Logseq 自带的设置面板（`Settings → Plugin Settings → DB Banner`，也可从弹层底部的链接打开）编辑的是同一批值：
