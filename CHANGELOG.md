...
- The icon opens the plugin's **own** settings popover instead of Logseq's generated
...
- The quote widget now shows the full quote whenever it fits, clipping only when its
  content reaches the banner's bound instead of clamping every quote to three lines.
