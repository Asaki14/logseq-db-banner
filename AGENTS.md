...
Then drive it over CDP (`CHROME_DEVTOOLS_AXI_BROWSER_URL=http://127.0.0.1:<port> chrome-devtools-axi …`; another agent's throwaway instance may already hold 9333 — the launch then logs `bind() failed: Address already in use` and CDP silently attaches to *their* app, so pick a free port and confirm `logseq.api.get_current_graph().path` is inside your own scratch before driving anything) and load the build with `LSPluginCore.unregister('db-banner')` followed by `LSPluginCore.register({ url: '<repo path>' })`; that pair is also how you reload after `npm run build`. From the host page, `logseq.api.<snake_case>` calls run directly; to test what the *plugin* sees, reach into its iframe (`document.querySelector('#db-banner_iframe').contentWindow`) — the plugin bridge has different serialisation limits than the host, and that window is also the way to read plugin-internal state: `chrome-devtools-axi console` returns only the newest few messages, so a `console.info` trace of what the plugin did is unreliable, while a temporary array on the iframe's `window` comes back whole. To change a plugin setting between runs, edit `<scratch>/home/.logseq/settings/db-banner.json` and re-register: the plugin cannot see its own `updateSettings` anyway. `chrome-devtools-axi eval` evaluates as an expression, so a snippet starting with `const`/`let` dies with `Unexpected token 'const'` — wrap it in `(() => { … })()` and return a JSON string. Layout claims (card heights, occupied area, column overflow) are worth measuring that way rather than eyeballing a screenshot: `getBoundingClientRect()` plus `scrollWidth === clientWidth` on the content column.
