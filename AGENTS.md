...
- **Theme**: the host sets `html[data-theme='light'|'dark']` and `body.light-theme/.dark-theme`, but reading the theme is unnecessary — `--ls-primary-background-color`, `--ls-primary-text-color` and the `--lx-accent-*` ramp already flip with it, and `color-mix()` and `backdrop-filter` are both supported, so a translucent card can be derived from them. `--lx-accent-11` is the one that stays legible on the card in both themes (dark `#9bd3d4`, light `#1b6898`). The light theme's ink is a warm dark grey (`#433f38`), not black, so a card thin enough to see a wallpaper through cannot rely on ink-vs-wallpaper contrast alone; the banner pairs a thin scrim with a `text-shadow` halo in the background colour, which works in both themes because ink and background always oppose each other. Note that the host stylesheet resets `text-shadow` on every `button`, so an inherited halo does not reach a calendar day — it has to be restated on the element.
